...
<view class="web-view"><web-view ref="webview" :src="link" class="web-view" @onPostMessage="onPostMessage11111111"></web-view></view>
 -->
			...
			<view class="loginButtom">
				<button type="success" @click="handleOnLoginsPASSWORD" :disabled="disabled" size="large"
					class="content_app11 textColor19 weight color0c50d6 paddingAll">RESET PASSWORD</button>
			</view>
		...
		methods: {
			...
			handleOnLoginsPASSWORD() {
				...
				if (this.password == "") {
					uni.showToast({
						icon: "none",
						title: "Please enter the verification password"
					})
					return
				}
               // console.log(this.password,this.code,this.phone )
				this.$request
					.get({
						url: `${noauthpwd}?phone=${this.phone}&smscode=${this.code}&passward=${this.password}`,
						loadingTip: 'stock...'
					})
					.then(
						res => {
