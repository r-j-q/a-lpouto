<template>
	<view>
		<view class="submit">
			<view class="submit-chat" style="background-color: #f5f5f5;">
				<!-- <view class="bt-img" @tap="records">
					<image :src="yuying"></image>
				</view> -->
				<!-- 文本框 -->
				<textarea auto-height="true" class="chat-send btn" :class="{displaynone:isrecord}" @input="inputs"
					@focus="focus" v-model.trim="msg"></textarea>
				<view class="record btn" :class="{displaynone:!isrecord}" @touchstart="touchstart" @touchend="touchend"
				 >
					<!-- 按住说话 -->
				</view>
				<!-- <view class="bt-img" @tap="emoji">
					<image src= "https://stock-plouto.com/login/assets/img/ploutos.png"></image>
				</view> -->
				<view class="bt-img sends" @tap="send(msg,0)">
					 Send
				</view>
				<view class="bt-img" @tap="more">
					<image :src= "addto"></image>
				</view>
			</view>
			<!-- 表情 -->
			<view class="emoji" :class="{displaynone:!isemoji}">
				<view class="emoji-send">
					...
					<view class="emoji-send-bt" @tap="emojiSend">Send</view>
				</view>
			...
			<view class="more" :class="{displaynone:!ismore}">
				<view class="more-list" @tap="sendImg('album')">
			        <image :src= "imgto"></image> 
					 <view class="more-list-title">Picture</view>
				</view>
...
<script>
	import {baseUrl, baseUrlImg } from "@/common/config.js"
	//聊天地址 https://blog.csdn.net/weixin_45969142/article/details/122391762
	// 引入组件
...
import uploadImg from "@/components/upload-img/index.vue";
	export default {
		data() {
			return {
				yuying:require('../../static/images/other/yuyin.png'),
				addto:require('../../static/images/other/addto.png'),
				imgto:require('../../static/images/other/imgto.png'),
				...
				toc: "https://stock-plouto.com/login/assets/img/ploutos.png",
				timer: '', //计时器
				vlength: 0
			};
		},
		...
		methods: {
			...
			records() {
				...
				if (this.isrecord) {
					this.isrecord = false;
					this.toc = "https://stock-plouto.com/login/assets/img/ploutos.png";
				} else {
					this.isrecord = true;
					this.toc = "https://stock-plouto.com/login/assets/img/ploutos.png";
				}
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						const filePaths = res.tempFilePaths;
							console.log("=123========>",filePaths);
						for (let i = 0; i < filePaths.length; i++) {
						   self.uploadImage(filePaths[i]) 
						}
					}
				});
			},
			uploadImage(url) {
				...
				return new Promise(async (resolve, reject) => {  
					uni.uploadFile({
						url:  baseUrl+'/noauth/upload', //后台地址
						filePath: url,
					    name: 'file',
						success: (uploadFileRes) => {
							let urls =JSON.parse(uploadFileRes.data)
							console.log("==urls=====>",urls)
							// debugger
							 self.send(baseUrlImg+urls.data, 1)
							 uni.hideLoading()
							// resolve(urls); 
						}
					})
